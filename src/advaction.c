...
/**
 * This describes the advection problem and time-stepping
 */
typedef struct ctx
{
 ...
 * \param [in]  ctx parameters
 *
 * \return the initial condition at \a x
 */
static double
initial_condition (double x[], double du[], ctx_t * ctx)
{
    int                 i;
    double              *c = ctx->center;
 ...
 */
static void
get_midpoint (p4est_t * p4est, p4est_topidx_t which_tree, p4est_quadrant_t * q, double xyz[3])
{
 ...
 */
static int
coarsen_initial_condition(p4est_t *p4est,
                          ...
                          p8est_quadrant_t **children)
{
    ...
    get_midpoint (p4est, which_tree, &parent, parentmidpoint);
                     ...
                     p8est_quadrant_t **children)
{
    ...
    h = (double) P4EST_QUADRANT_LEN (children[0]->level) /
    ...
    for (i = 0; i < P4EST_CHILDREN; i++) {
        data = (data_t *) children[i]->p.user_data;
              ...
              int num_incoming, p8est_quadrant_t **incoming)
{
    ...
    if (num_outgoing > 1) {
        /* this is coarsening */
        parent_data = (data_t *) incoming[0]->p.user_data;
        parent_data->u = 0.;
        for (j = 0; j < P4EST_DIM; j++) {
            parent_data->du[j] = invalid;

        }
        for (i = 0; i < P4EST_CHILDREN; i++) {
            child_data = (data_t *) outgoing[i]->p.user_data;
        ...
        /* this is refinement */
        parent_data = (data_t *) outgoing[0]->p.user_data;
        h =
                (double) P4EST_QUADRANT_LEN (outgoing[0]->level) /
                (double) P4EST_ROOT_LEN;

        for (i = 0; i < P4EST_CHILDREN; i++) {
            child_data = (data_t *) incoming[i]->p.user_data;
 ...
 */
static void
write_solution(p4est_t *p4est, int timestep)
{
 ...
 * always 0.
 *
 * Like interpolate_solution(), this function matches the
 ...
 */
static void
quad_divergence(p4est_iter_volume_info_t *info, void *user_data)
{
    p4est_quadrant_t    *q = info->quad;
    data_t              *data = (data_t *) q->p.user_data;
    ...
    if (side[upwindside]->is_hanging) {
        ...
        for (j = 0; j < P4EST_HALF; j++) {
            if (side[upwindside]->is.hanging.is_ghost[j]) {
                udata = (data_t *) &ghost_data[side[upwindside]->is.hanging.quadid[j]];
            }
            else {
                udata = (data_t *) side[upwindside]->is.hanging.quad[j]->p.user_data;
            }
            uavg += udata->u;
        }
        uavg /= P4EST_HALF;
        ...
        if (side[upwindside]->is.full.is_ghost) {
            udata = (data_t *) & ghost_data[side[upwindside]->is.full.quadid];
        }
        else {
            udata = (data_t *) side[upwindside]->is.full.quad->p.user_data;
        }
        uavg = udata->u;
    }
 ...
 * quad_divergence() and step3_upwind_flux(). Now we multiply this by
...
minmod_estimate(p4est_iter_face_info_t *info, void *user_data)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (side[i]->is_hanging) {
            ...
            for (j = 0; j < P4EST_HALF; j++) {
                quad = side[i]->is.hanging.quad[j];
                h[i] = (double) P4EST_QUADRANT_LEN (quad->level) / (double) P4EST_ROOT_LEN;
                if (side[i]->is.hanging.is_ghost[j]) {
 ...
 */
static void
compute_max(p4est_iter_volume_info_t *info, void *user_data)
{
    p4est_quadrant_t    *q = info->quad;
    data_t              *data = (data_t *) q->p.user_data;
...
get_timestep(p4est_t *p4est)
{
    ...
    min_h = (double) P4EST_QUADRANT_LEN (global_max_level) / (double) P4EST_ROOT_LEN;
 ...
 */
static void
timestep(p4est_t *p4est, double time)
{
    double              t = 0.;
    double              dt = 0.;
    int                 i;
    ...
    for (t = 0., i = 0; t < time; t += dt, i++) {
        ...
        if (!(i % refine_period)) {
            if (i) {
                ...
                p4est_iterate(p4est, NULL, (void *) &umax,     /* pass in ghost data that we just exchanged */
                              compute_max,       /* blank the previously calculated derivatives */
                              NULL,    /* there is no callback for the faces between quadrants */
                              NULL,    /* there is no callback for the edges between quadrants */
                              NULL);   /* there is no callback for the corners between quadrants */

                mpiret = sc_MPI_Allreduce (&umax, &global_umax, 1, sc_MPI_DOUBLE, sc_MPI_MAX,
                                          p4est->mpicomm);
                SC_CHECK_MPI (mpiret);
                ...
                p4est_refine_ext(p4est, recursive, allowed_level,
                                 refine_err_estimate, NULL,
                                 replace_quads);
                p4est_coarsen_ext(p4est, recursive, callbackorphans,
                                  coarsen_err_estimate, NULL,
                                  replace_quads);
        ...
        p4est_iterate(p4est,                 /* the forest */
                      ...
                      quad_divergence, /* callback to compute each quad's
                                             interior contribution to du/dt */
                      upwind_flux,     /* callback to compute each quads'
        ...
        p4est_iterate(p4est, NULL, /* ghosts are not needed for this loop */
                      (void *) &dt,        /* pass in dt */
                      timestep_update,       /* update each cell */
                      NULL,        /* there is no callback for the faces between quadrants */
                      NULL,        /* there is no callback for the edges between quadrants */
                      NULL);       /* there is no callback for the corners between quadrants */
...
main (int argc, char **argv)
{
    ...
    p4est = p4est_new_ext(mpicomm, /* communicator */
                          ...
                          init_initial_condition,  /* initializes data */
                          (void *) (&ctx));              /* context */
    ...
    p4est_coarsen(p4est, recursive, coarsen_initial_condition,
                  init_initial_condition);

    p8est_vtk_write_file (p4est, NULL, "advaction_init");
